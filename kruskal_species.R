#Performs an enrichment analysis between plant compartments per each plants species
#Please indicate below the Domain you want to analyze and the taxa rank
#Please set endo for endopsheri compartents or epis for epispheric compartments
...
microbe=c("Bacteria","Fungi")[c(1)]
#Chose 3 compartments only 
compartment=c("soil","roo.zone.soil","root.endosphere","rhizosphere","leaf.endosphere","phyllosphere")[c(1,3,5)]
...
for (j in plant){
  ...
  result.frame=data.frame(row.names = taxas) #generates result data frame of pvalues
    ...
    cam2=subset_samples(cam1, Sample %in% compartment) %>%
    ...
    otu.tidy$Sample=factor(otu.tidy$Sample) #Change strings to factors 
    ...
    for (h in as.character(unique(otu.frame$lineage))){
      ...
      kruskal[h,"p.value"]=kruskal.test(relative.abundance ~ Sample, data = sub1)$p.value
    }
    ### 
    ...
    if (sum(kruskal$p.value<=0.05)>0){
      ...
      for (h in as.character(unique(otu.frame$lineage))){
        sub1=otu.tidy[otu.tidy$lineage==h,]
        test1=dunnTest(relative.abundance ~ Sample, data = sub1, method = "bh")
        p.value=test1$res$P.adj
        dunn[h,1:choose(length(compartment),2)]=p.value
      }
      colnames(dunn)=test1$res$Comparison
      dunn[is.na(dunn)]=1
      
      
      write.table(dunn,paste(result,paste(microbe,m,"Pvalues",j,grupo,"txt",sep="."),sep = "/")[1], 
    ...
    otu.se=summarySE(data=otu.tidy, measurevar = "relative.abundance", groupvars = c("Sample","lineage"))
    otu.dif=data.frame(row.names=unique(otu.se$lineage),
                       V1=otu.se[otu.se$Sample==compartment[3],"relative.abundance"]-otu.se[otu.se$Sample==compartment[2],"relative.abundance"],
                       V2=otu.se[otu.se$Sample==compartment[3],"relative.abundance"]-otu.se[otu.se$Sample==compartment[1],"relative.abundance"],
                       V1=otu.se[otu.se$Sample==compartment[2],"relative.abundance"]-otu.se[otu.se$Sample==compartment[1],"relative.abundance"])
    otu.dif[otu.dif>0]=1 
    otu.dif[otu.dif<0]=-1 
    colnames(otu.dif)=test1$res$Comparison
    
    for (h in as.character(unique(otu.frame$lineage))){ #merges de direction of the enrichment
      log.frame[h,]= log.frame[h,]*otu.dif[h,] 
      }

    write.table(log.frame,paste(result,paste(microbe,m,"logPvalues",j,grupo,"txt",sep="."),sep = "/")[1], 
                quote = F, row.names = T, col.names = T, sep = "\t")
