...
compartment=c("soil","roo.zone.soil","rhizosphere",
                 "root.endosphere","leaf.endosphere","phyllosphere")[c(5,6)]
...
if (plant == "Agave.salmiana" & comp=="leaf.endosphere"){
  #Enrichment by compartment and species
  ...
  nodes$root.endosphere=ifelse(nodes$OTU.ID %in%  rownames(cdiff[cdiff[,"leaf.endosphere...root.endosphere"] >= -log10(0.05),]),"enriched","not.enriched")
  nodes$soil=ifelse(nodes$OTU.ID %in%  rownames(cdiff[cdiff[,"leaf.endosphere...soil"] >= -log10(0.05),]),"enriched","not.enriched")
  #OTUS enriched in compartments
  ...
  vertex_attr(g, name="root", index = V(g)) <- nodes$root.endosphere
...
if (plant == "Agave.salmiana" & comp=="phyllosphere"){
  #Enrichment by compartment and species
  ...
  nodes$rainy=ifelse(nodes$OTU.ID %in%  rownames(sdiff[sdiff[,grupo] <= log10(0.05),]),"enriched","not.enriched")
  #OTUS enriched in compartments
  nodes$rhizosphere=ifelse(nodes$OTU.ID %in%  rownames(cdiff[cdiff[,"phyllosphere...rhizosphere"] >= -log10(0.05),]),"enriched","not.enriched")
  nodes$soil=ifelse(nodes$OTU.ID %in%  rownames(cdiff[cdiff[,"phyllosphere...soil"] >= -log10(0.05),]),"enriched","not.enriched")
  #OTUS enriched in compartments
  nodes$Agave.tequilana=ifelse(nodes$OTU.ID %in% rownames(pdiff[pdiff[,"Agave.salmiana...Agave.tequilana"] >= -log10(0.05),]),"enriched","not.enriched")
  nodes$Cacti=ifelse(nodes$OTU.ID %in% rownames(pdiff[pdiff[,"Agave.salmiana...Cacti"] >= -log10(0.05),]),"enriched","not.enriched")
  #Asign network atributes
...
if (plant == "Agave.tequilana" & comp=="leaf.endosphere"){
  #Enrichment by compartment and species
  cdiff=rbind(read.delim(paste(diff.dir,paste("Bacteria","OTU.ID","logPvalues",plant,"endo","txt",sep="."),sep = "/")[1]),
              read.delim(paste(diff.dir,paste("Fungi","OTU.ID","logPvalues",plant,"endo","txt",sep="."),sep = "/")[1]))
  #OTUs enriched in seasons
...
if (plant == "Agave.tequilana" & comp=="phyllosphere"){
  #Enrichment by compartment and species
  cdiff=rbind(read.delim(paste(diff.dir,paste("Bacteria","OTU.ID","logPvalues",plant,"epis","txt",sep="."),sep = "/")[1]),
              read.delim(paste(diff.dir,paste("Fungi","OTU.ID","logPvalues",plant,"epis","txt",sep="."),sep = "/")[1]))
  #OTUs enriched in seasons
...
if (plant == "Cacti" & comp=="leaf.endosphere"){
  #Enrichment by compartment and species
  cdiff=rbind(read.delim(paste(diff.dir,paste("Bacteria","OTU.ID","logPvalues",plant,"endo","txt",sep="."),sep = "/")[1]),
              read.delim(paste(diff.dir,paste("Fungi","OTU.ID","logPvalues",plant,"endo","txt",sep="."),sep = "/")[1]))
  #OTUs enriched in seasons
...
if (plant == "Cacti" & comp=="phyllosphere"){
  #Enrichment by compartment and species
  cdiff=rbind(read.delim(paste(diff.dir,paste("Bacteria","OTU.ID","logPvalues",plant,"epis","txt",sep="."),sep = "/")[1]),
              read.delim(paste(diff.dir,paste("Fungi","OTU.ID","logPvalues",plant,"epis","txt",sep="."),sep = "/")[1]))
  #OTUs enriched in seasons
  ...
  result2=data.frame(row.names = c("Nodes","Hub","Bacteria.E","Fungal.E","Dry","Rainy","vs Root","vs Soil", "vsplant1","vsplant2"))
  ...
  result2t$enrichment=factor(result2t$enrichment,levels=c("Nodes","Hub","Bacteria.E","Fungal.E","Dry","Rainy","vs Root","vs Soil", "vsplant1","vsplant2"))
